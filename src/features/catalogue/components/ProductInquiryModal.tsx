                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="flex flex-wrap -mx-2.5">
                    {fields.map((f: any, idx: number) => {
                      ...
                      const fieldKeyId = f.key || `field_${idx}`;
                      
                      return (
                        <div key={fieldKeyId} className={`px-2.5 mb-5 ${isHalf ? 'w-full md:w-1/2' : 'w-full'}`}>
                            ...
                            />
                             ...
                             >
                               ...
                               {(f.options || []).map((o: any, oIdx: number) => {
                                 ...
                                 return <option key={o.key || `opt_${oIdx}`} value={optLabel}>{optLabel}</option>
                               })}
                             </select>
                          ) : (
                            <input 
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-gray-100 mt-6 pt-5">
                    {(() => {
                      const rawNumber = dbData?.inquiryModal?.whatsappNumber || "84932058545";
                      const cleanNumber = rawNumber.replace(/[^0-9]/g, '');
                      return (
                        <a 
                          href={`https://wa.me/${cleanNumber}?text=Hi, I'd like to inquire about ${encodeURIComponent(pName)} (${product.code})`} 
                          target="_blank" 
