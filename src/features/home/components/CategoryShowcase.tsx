                      ...
                      {cat.subItems ? (
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {cat.subItems.map((sub) => (
                            <span key={sub} className="font-body text-[9px] tracking-wide uppercase px-1.5 py-0.5 rounded-sm bg-white/10 text-white/60">
                              {t(`home.categories.${cat.key}.sub.${sub}`)}
                            </span>
                          ))}
                        </div>
                      ) : (
