        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 xl:gap-24 items-center">
          <div>
            ...
            <motion.div initial={{ opacity: 0, y: 20, filter: "blur(4px)" }} whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }} viewport={vp} transition={{ duration: 0.65, delay: 0.28, ease }} className="space-y-3 sm:space-y-4 mb-8 sm:mb-10 max-w-lg">
              <p className="font-body text-sm sm:text-base leading-relaxed" style={{ color: "hsl(var(--muted-foreground))" }}>DHT Outdoor Furniture Production là nhà sản xuất nội thất ngoài trời cao cấp với cơ sở sản xuất 5,000m² tại Jakarta. Hơn 15 năm hợp tác với các thương hiệu khách sạn, resort và nhà thiết kế trên 25+ quốc gia.</p>
              <p className="font-body text-sm sm:text-base leading-relaxed" style={{ color: "hsl(var(--muted-foreground))" }}>Mỗi sản phẩm — từ bộ bàn ghế teak đến bộ sofa chống thời tiết — được chế tác bởi 400+ nghệ nhân lành nghề, trải qua kiểm soát chất lượng đa tầng.</p>
            </motion.div>
