        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 xl:gap-24 items-center">
          <div>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={vp} transition={{ duration: 0.5, delay: 0.44, ease }} className="flex flex-col sm:flex-row items-center gap-4">
              <Link href="/catalogue" className="inline-flex items-center justify-center px-6 py-3 font-body font-semibold text-sm text-white rounded-sm hover:opacity-90 transition-all text-center w-full sm:w-auto" style={{ backgroundColor: "hsl(var(--orange))" }}>
          ...
          <div className="flex flex-col gap-4">
            <motion.div initial={{ opacity: 0, scale: 0.97, filter: "blur(8px)" }} whileInView={{ opacity: 1, scale: 1, filter: "blur(0px)" }} viewport={vp} transition={{ duration: 1, delay: 0.1, ease }} className="relative flex gap-3 h-64 sm:h-80 lg:h-96">
              {[
                "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&auto=format&fit=crop&q=80",
                "https://images.unsplash.com/photo-1595526114101-1b702ecacccf?w=600&auto=format&fit=crop&q=80",
                "https://images.unsplash.com/photo-1581428982868-e410dd047a90?w=600&auto=format&fit=crop&q=80"
              ].map((src, idx) => (
                <div key={idx} className="flex-1 rounded-sm overflow-hidden h-full group">
                  <img src={src} alt="Factory Process" className="w-full h-full object-cover transition-transform duration-700 hover:scale-[1.1]" />
                </div>
              ))}
              <div className="absolute inset-0 bg-gradient-to-t from-[hsl(var(--navy-deep)/0.3)] to-transparent pointer-events-none" />
              <motion.div initial={{ opacity: 0, x: 14 }} whileInView={{ opacity: 1, x: 0 }} viewport={vp} transition={{ duration: 0.45, delay: 0.65, ease }} className="absolute top-4 right-4 px-3 py-1.5 rounded-full z-10" style={{ backgroundColor: "hsl(var(--orange))" }}>
