    ...
    <motion.div layout initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 16, scale: 0.97 }} transition={{ duration: 0.55, delay: index * 0.06, ease: [0.16, 1, 0.3, 1] }} className="group cursor-pointer" onClick={() => onQuickView(product)}>
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center justify-between pt-0.5">
          ...
          <span className="font-body text-xs font-semibold flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200" style={{ color: "hsl(var(--orange))" }}>Inquire <ArrowRight size={11} /></span>
        </div>
      </div>
    </motion.div>
  );
    ...
    <section className="py-16 sm:py-28" style={{ backgroundColor: "hsl(var(--warm-cream))" }}>
      <div ref={ref}>
        <div className="container mx-auto px-4 sm:px-6 md:px-8">
          ...
          {filtered.length === 0 && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20"><p className="font-body text-muted-foreground">No featured products in this category yet.</p></motion.div>)}
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="flex justify-center mt-16">
            <Link href="/catalogue" className="group relative inline-flex items-center gap-3 px-10 py-4 rounded-sm font-body font-semibold text-sm overflow-hidden transition-all duration-300 border-2" style={{ borderColor: "hsl(var(--orange))", color: "hsl(var(--orange))" }}>
              ...
              <span className="relative z-10 group-hover:text-white transition-colors duration-200 flex items-center gap-3">See All Products <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform duration-200" /></span>
            </Link>
          </motion.div>
        </div>
      </div>
