...
export default function Testimonials() {
  ...
  const testimonials = t("home.testimonials.quotes", { returnObjects: true }) as Array<{ quote: string; name: string; role: string; project: string }>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          <motion.div variants={stagger(0, 0.12)} initial="hidden" animate={vis} className="lg:col-span-4">
            <motion.div variants={fadeUp} className="flex items-center gap-3 mb-6">
              ...
              <span className="font-body text-xs tracking-[0.25em] uppercase font-medium" style={{ color: "hsl(var(--orange))" }}>{t("home.testimonials.label")}</span>
            </motion.div>
            <motion.h2 variants={fadeUp} className="font-display font-bold text-foreground leading-tight mb-6" style={{ fontSize: "clamp(2rem, 4vw, 3rem)" }}>{t("home.testimonials.heading")}</motion.h2>
            <div className="flex flex-col gap-3">
              {testimonials.map((item, i) => (
                <button key={i} onClick={() => setActive(i)} className={`flex items-center gap-3 p-3 rounded-sm text-left transition-all ${i === active ? "shadow-sm opacity-100" : "opacity-60 hover:opacity-80"}`} style={i === active ? { backgroundColor: "hsl(var(--warm-cream))", borderLeft: "3px solid hsl(var(--orange))" } : {}}>
                  <img src={avatars[i]} alt={item.name} className="w-10 h-10 rounded-full object-cover shrink-0" />
                  <div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.5 }} className="lg:col-span-8 relative p-5 sm:p-8 rounded-sm" style={{ backgroundColor: "hsl(var(--warm-cream))" }}>
              ...
              <p className="font-display font-medium leading-relaxed text-foreground mb-8 relative z-10" style={{ fontSize: "clamp(1.1rem, 2vw, 1.4rem)" }}>&ldquo;{tt.quote}&rdquo;</p>
              <div className="flex items-center gap-4">
                <img src={avatars[active]} alt={tt.name} className="w-12 h-12 rounded-full object-cover border-2" style={{ borderColor: "hsl(var(--orange))" }} />
                <div>
