...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    try {
      ...
      const revision = await ContactRevision.findById(revisionId);
      if (!revision) {
        return res.status(404).json({ error: "Revision not found" });
      }

      // Clear previous defaults
      await ContactRevision.updateMany({ isDefault: true }, { $set: { isDefault: false } });
  ...
  if (req.method === "GET") {
    try {
      const defaultRev = await ContactRevision.findOne({ isDefault: true }).lean();
