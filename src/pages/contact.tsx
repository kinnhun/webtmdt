...
export default function ContactPage() {
  ...
  const contactInfo = [
    { icon: Phone, label: t("contact.info.phone"), value: "+62 21 234 567 890", href: "tel:+6221234567890" },
    { icon: Mail, label: t("contact.info.email"), value: "info@dht-furniture.com", href: "mailto:info@dht-furniture.com" },
    { icon: MapPin, label: t("contact.info.factory"), value: "12 Commerce Blvd, Jakarta 12345" },
    { icon: Clock, label: t("contact.info.hours"), value: t("contact.info.hoursValue") },
    { icon: Globe, label: t("contact.info.export"), value: t("contact.info.exportValue") },
  ];
    ...
    <>
      ...
      <div className="pt-20" style={{ minHeight: "100vh", backgroundColor: "hsl(var(--warm-cream))" }}>
        <div className="py-16 text-center" style={{ backgroundColor: "hsl(var(--navy-deep))" }}>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="font-display font-bold text-white mb-3" style={{ fontSize: "clamp(2rem, 5vw, 3.5rem)" }}>{t("contact.hero.title")}</motion.h1>
          <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="font-body text-white/50 text-sm max-w-md mx-auto">{t("contact.hero.subtitle")}</motion.p>
        </div>
